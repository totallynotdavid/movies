...
const headline = computed(() => {
  ...
  const lean =
    weekendPct >= 60
      ? " — your watching leans to the weekend"
      : weekendPct <= 25
        ? " — you're a weeknight watcher"
        : "";
...
const rhythm = computed(() => {
  ...
  if (nightOwlShare >= NIGHT_OWL_SHARE) {
    return `A night owl — ${Math.round(nightOwlShare * 100)}% of watches land after midnight.`;
  }
  if (peakHour !== null) {
...
<template>
  ...
  >
    <div class="flex flex-col gap-1">
      <h2 class="text-sm font-mono text-fg-muted">patterns</h2>
      <p class="text-sm text-fg-subtle">{{ headline }}</p>
    </div>
      ...
      >
        <div class="flex h-24 w-full items-end">
          <div
            ...
            :class="day.weekday === weekday.busiest?.weekday ? 'bg-accent' : 'bg-accent/30'"
