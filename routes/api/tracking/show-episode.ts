...
import { logShowEpisode } from "../../../src/domain/activity";
...
export const POST = defineHandler(async (c) => {
  ...
  try {
    const entry = await logShowEpisode({
      userId: user.id,
    ...
    const message = error instanceof Error ? error.message : "Failed to log episode";
